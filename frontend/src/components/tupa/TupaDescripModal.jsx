...
export const TupaDescripModal = ({ openModal, setOpenModal }) => {
  ...
  const updateDescrip = useTupaStore((state) => state.updateDescrip);
  ...
  const textDescrip = useRef(null);

  const { tupaId, tupaDescrip } = currentTupa;

  const onClickUpdateDescrip = async () => {
    try {
      console.log("Se inicia onClickUpdateDescrip");
      console.log(textDescrip.current.value);
      await updateDescrip(tupaId, textDescrip.current.value);
